...
function DropDownList:Constructor()
	...
	self.CurrentValue=Turbine.UI.TextBox();
--	self.CurrentValue=Turbine.UI.Label();
	...
	self.CurrentValue:SetMultiline(false)
	self.ValueMask=Turbine.UI.Control()
	self.ValueMask:SetParent(self)
	self.ValueMask:SetSize(self.CurrentValue:GetSize())
	self.ValueMask:SetPosition(self.CurrentValue:GetPosition())
	...
	self.MatchType=0 -- 0=no match, current not shown; 1=match starting at left; 2=match on contains
	...
	self.ValueMask.MouseClick=function(sender,args)
		self.CurrentValue.MouseClick(self.CurrentValue,args)
	end
...
function DropDownList:SetMatchType(type)
	if type==nil then type=0 end
	if type==0 then
		self.CurrentValue:SetSelectable(false)
		self.CurrentValue.TextChanged=nil
		self.CurrentValue.FocusGained=nil
	elseif type==1 or type==2 then
		self.CurrentValue:SetSelectable(true)
		self.MatchType=type
		self.CurrentValue.TextChanged=function(sender,args)
			-- be sure that the list is showing
			local tmpDDL=sender:GetParent()
			tmpDDL:ShowList()
			-- allow the user to try to select from the list by typing a matching name
			local tmpVal=string.lower(string.stripaccent(sender:GetText()))
			local tmpLen=string.len(tmpVal)
			if tmpVal~="" then
				local match
				for k=1,tmpDDL.ListData:GetItemCount() do
					local tmpMap=string.stripaccent(tmpDDL.ListData:GetItem(k):GetText())
					tmpMap=string.lower(string.ltrim(tmpMap))
					local success,result
					if tmpMap==tmpVal then
						match=k
						break
					else
						-- allow search to continue if match found but was not exact in case there is an exact further down list
						if match==nil then
							if tmpDDL.MatchType==1 then
								-- match on contains
								success,result=pcall(string.find,tmpMap,tmpVal);
								if result~=nil then
									match=k
								end
							elseif tmpDDL.MatchType==2 then
								-- match on starts with
								if string.sub(tmpMap,1,string.len(tmpVal))==tmpVal then
									match=k
								end
							end
						end
				...
				tmpDDL:ShowEntry(match)
				tmpDDL.ListData:EnsureVisible(match)
			end
		end
		self.CurrentValue.Update=function()
			local len=string.len(self.CurrentValue:GetText())
			self.CurrentValue:SetSelection(1,len)
			self.CurrentValue:SetWantsUpdates(false)
		end
		self.CurrentValue.FocusGained=function()
			self.CurrentValue:SetWantsUpdates(true)
		end
	else
		Turbine.Shell.WriteLine("Invalid Match Type for DropDownList. Reset to Type=0")
		self.MatchType=0
	end
end
...
function DropDownList:SetSelectedIndex(index)
	if index~=nil and index>0 and index<=self.ListData:GetItemCount() then
		local listItem=self.ListData:GetItem(index)
		self.CurrentValue:SetText(listItem:GetText())
		self.CurrentValue.DataValue=listItem.DataValue;
		self:HideList();
		self.ListData:SetSelectedIndex(listItem.Index);
		if self.SelectedIndexChanged~=nil then
			self.SelectedIndexChanged(self)
		end
	end
end
