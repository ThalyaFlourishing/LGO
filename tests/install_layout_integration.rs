...
/// need (base-stat derivations and the items DB). These are minimal synthetic
/// fixtures — these tests exercise install-layout routing, discovery, and CLI
/// behaviour, not the real fixture contents.
fn seed_install(tag: &str) -> PathBuf {
    let install = unique_dir(tag);
    let data = install.join("data");
    common::write_derivations(&data);
    common::write_items(&data);
    install
}
