...
//!
//! Output: a `PluginExport` extracted from `lgo_gearlist_*.plugindata`,
//! containing character metadata used by the optimizer.
...
mod tests {
    ...
    #[test]
    fn extracts_minimal_gearlist_export() {
        let src = r#"{
        ...
        let val = parse(src);
        let export = extract_export(val).expect("extract export");
