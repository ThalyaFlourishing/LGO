...
use toml_edit::{value, Array, ArrayOfTables, Decor, DocumentMut, Item, Table, Value};
use unicode_normalization::{is_nfc, UnicodeNormalization};

use crate::base_stats::DerivationError;
use crate::gear::{parse_slot_display, Slot};
use crate::gearstats::{
    MeasuredStats, MEASURED_ACTIVE_EFFECTS_KEY, MEASURED_EQUIPPED_KEY, MEASURED_FIELD_KEYS,
    MEASURED_MAX_MORALE_KEY, MEASURED_MAX_POWER_KEY, MEASURED_TABLE_KEY,
};
use crate::stat::{Stat, BASE_STATS, TRACKED_STATS};
use crate::virtues::{
    VIRTUE_FIELD_KEYS, VIRTUE_INNATE_DEFAULT, VIRTUE_INNATE_FIELDS, VIRTUE_INNATE_TABLE_KEY,
    VIRTUE_TABLE_KEY,
};
...
struct ResolveMetadata<'a> {
    ...
    /// Character level from the plugin export; `None` when the export has
    /// none (old `lgo-gearlist-1` format), in which case no `level` is written.
    level: Option<u32>,
    /// Measured Max Morale/Power plus the equipped set they were measured
    /// with; `None` when the export lacks those fields.
    measured: Option<&'a MeasuredStats>,
}
...
/// Write `character`, `class`, `level`, `[InnateStats]`, `[MeasuredStats]`,
/// and `[Virtues]` into the document header.
///
/// `[InnateStats]` carries the character's five *raw* Base stats from the
/// plugindata export — no derivation happens in the resolver.
/// `[MeasuredStats]` carries the measured Max Morale/Power, the active-effect
/// count, and the equipped item names from the same export; like `level`, it
/// is generated and rebuilt wholesale on every run. `[Virtues]` carries the
/// user's five fixed Virtue selection slots as empty strings for later
/// hand-editing. Derivation into tracked stats is the optimize path's job;
/// see the pass-through design in the repo docs.
fn apply_top_level_metadata(doc: &mut DocumentMut, metadata: &ResolveMetadata<'_>) {
    ...
    // Root-level values always serialize before any table, so `level` needs
    // no position handling — only ordering relative to `character`/`class`.
    match metadata.level {
        Some(level) => {
            doc.insert(LEVEL_KEY, value(i64::from(level)));
        }
        None => {
            doc.remove(LEVEL_KEY);
        }
    }
    ...
    match metadata.measured {
        Some(measured) => {
            doc.insert(
                MEASURED_TABLE_KEY,
                Item::Table(build_measured_stats_table(measured)),
            );
        }
        None => {
            doc.remove(MEASURED_TABLE_KEY);
        }
    }
    doc.insert(VIRTUE_TABLE_KEY, Item::Table(build_virtues_table()));
    doc.insert(
        VIRTUE_INNATE_TABLE_KEY,
        Item::Table(build_virtue_innate_stats_table()),
    );
}

/// Pin the generated header tables to the front of the document, in canonical
/// order: `[InnateStats]` = 0, `[MeasuredStats]` = 1, `[Virtues]` = 2,
/// `[VirtueInnateStats]` = 3, and the `[[item]]` tables from 4 upward.
///
/// toml_edit serializes top-level tables by their internal `position` index,
/// not by map insertion order, and `push_group` renumbers every `[[item]]`
/// table from 0 — so the header tables must be repositioned explicitly, or
/// they drift a little further on every run (Bug 10). Gaps left by an absent
/// header table are harmless: positions only have to be ordered.
fn reorder_resolved_header_before_items(doc: &mut DocumentMut) {
    const INNATE_POSITION: usize = 0;
    const MEASURED_POSITION: usize = 1;
    const VIRTUES_POSITION: usize = 2;
    const VIRTUE_INNATE_POSITION: usize = 3;
    const FIRST_ITEM_POSITION: usize = 4;

    for (key, position) in [
        ("InnateStats", INNATE_POSITION),
        (MEASURED_TABLE_KEY, MEASURED_POSITION),
        (VIRTUE_TABLE_KEY, VIRTUES_POSITION),
        (VIRTUE_INNATE_TABLE_KEY, VIRTUE_INNATE_POSITION),
    ] {
    ...
    if let Some(items) = doc.get_mut("item").and_then(|i| i.as_array_of_tables_mut()) {
        for (offset, table) in items.iter_mut().enumerate() {
            table.set_position(offset + FIRST_ITEM_POSITION);
        }
    }
}
...
/// Build the `[VirtueInnateStats]` block: the two virtue-passive mitigation
/// values seeded to `VIRTUE_INNATE_DEFAULT`. Like `[Virtues]`, the block is
/// user-maintained — this only supplies the initial defaults; existing values
/// are preserved by `ensure_virtue_innate_fields` on subsequent runs.
fn build_virtue_innate_stats_table() -> Table {
    let mut table = Table::new();
    for (_, key) in VIRTUE_INNATE_FIELDS {
        table.insert(key, value(VIRTUE_INNATE_DEFAULT));
        normalize_assignment_decor(&mut table, key);
    }
    ensure_table_header_note(&mut table, VIRTUE_INNATE_NOTE);
    table
}
...
/// Build the generated `[MeasuredStats]` block from one plugin export: the
/// three counters as plain integers and `Equipped` as a single-line array of
/// item names in slot order (duplicates preserved). Rebuilt wholesale on
/// every resolve/merge, like the `two_handed` refresh — nothing in this block
/// is hand-edited, so nothing in it is preserved.
fn build_measured_stats_table(measured: &MeasuredStats) -> Table {
    let mut table = Table::new();
    table.insert(MEASURED_MAX_MORALE_KEY, value(measured.max_morale));
    table.insert(MEASURED_MAX_POWER_KEY, value(measured.max_power));
    table.insert(
        MEASURED_ACTIVE_EFFECTS_KEY,
        value(i64::from(measured.active_effects)),
    );
    let equipped: Array = measured.equipped.iter().map(String::as_str).collect();
    table.insert(MEASURED_EQUIPPED_KEY, Item::Value(Value::Array(equipped)));
    for key in MEASURED_FIELD_KEYS {
        normalize_assignment_decor(&mut table, key);
    }
    ensure_table_header_note(&mut table, MEASURED_STATS_NOTE);
    table
}
...
/// Ensure the two `[VirtueInnateStats]` fields exist, seeding missing ones to
/// `VIRTUE_INNATE_DEFAULT` while preserving existing values verbatim, then
/// normalize alignment and the header note. Mirrors `ensure_virtue_fields`:
/// the block is user-maintained, so an existing value is never overwritten.
fn ensure_virtue_innate_fields(table: &mut Table) {
    for (_, key) in VIRTUE_INNATE_FIELDS {
        if table.get(key).is_none() {
            table.insert(key, value(VIRTUE_INNATE_DEFAULT));
        }
    ...
    ensure_table_header_note(table, VIRTUE_INNATE_NOTE);
}
...
pub fn merge_into_canonical(
    ...
    // `level` and `[MeasuredStats]` are generated from the plugin export and
    // rebuilt wholesale on every merge — nothing in them is hand-edited, so
    // nothing in them is preserved. When the export drove this resolve but
    // carried no measured fields (an old `lgo-gearlist-1` export), the stale
    // values are removed rather than left to masquerade as current truth.
    ...
    match incoming_doc.get(LEVEL_KEY).cloned() {
        ...
        None if export_driven => {
            prev_doc.remove(LEVEL_KEY);
        }
        None => {}
    }
    match incoming_doc
        ...
        .cloned()
    {
        Some(incoming_measured) => {
            prev_doc.insert(MEASURED_TABLE_KEY, Item::Table(incoming_measured));
        }
        None if export_driven => {
            prev_doc.remove(MEASURED_TABLE_KEY);
        }
        None => {}
    }
    ...
    // Carry forward or create `[VirtueInnateStats]`. Like `[Virtues]`, these
    // two virtue-passive mitigation values are user-maintained: preserve
    // existing values verbatim, seed any missing field to the default, and
    // normalize alignment. Gated on the same predicate as `[Virtues]`.
    if should_have_virtues {
        match prev_doc
            .get_mut(VIRTUE_INNATE_TABLE_KEY)
            .and_then(|i| i.as_table_mut())
        {
            Some(prev_virtue_innate) => ensure_virtue_innate_fields(prev_virtue_innate),
            None => {
                let mut virtue_innate = incoming_doc
                    .get(VIRTUE_INNATE_TABLE_KEY)
                    .and_then(|i| i.as_table())
                    .cloned()
                    .unwrap_or_else(build_virtue_innate_stats_table);
                ensure_virtue_innate_fields(&mut virtue_innate);
                prev_doc.insert(VIRTUE_INNATE_TABLE_KEY, Item::Table(virtue_innate));
            }
        }
    }
...
pub(crate) fn nfc_name(name: &str) -> Cow<'_, str> {
    if is_nfc(name) {
        Cow::Borrowed(name)
    } else {
...
/// Build the `[MeasuredStats]` payload from one plugin export. Both measured
/// maxima have to be present for the block to mean anything; an export that
/// lacks them (the old `lgo-gearlist-1` shape) yields `None`, which removes
/// any stale block instead of writing partial data.
fn plugin_measured_stats(export: &crate::plugindata::PluginExport) -> Option<MeasuredStats> {
    let (Some(max_morale), Some(max_power)) = (export.max_morale, export.max_power) else {
        return None;
    };
    Some(MeasuredStats {
        max_morale,
        max_power,
        active_effects: export.active_effects.unwrap_or(0),
        equipped: export.equipped.clone(),
    })
}
...
pub fn resolve_stats_file(
    ...
    let level = plugin_export.as_ref().and_then(|export| export.level);
    let measured = plugin_export.as_ref().and_then(plugin_measured_stats);
...
mod tests {
    ...
    /// `docs/AGENT_CONTEXT.md` §5: root values first, then the four header
    /// tables at positions 0/1/2/3, then the items.
    fn assert_canonical_header_order(src: &str) {
        ...
        let virtue_innate = src
            .find("[VirtueInnateStats]")
            .expect("VirtueInnateStats block exists");
        ...
        assert!(
            character < class
                ...
                && innate < measured
                && measured < virtues
                && virtues < virtue_innate
                && virtue_innate < divider,
            "header must read character, class, level, [InnateStats], [MeasuredStats], \
             [Virtues], [VirtueInnateStats], first divider:\n{}",
            src
        );
    }
...
name = \"Test Helm\"\n";
    ...
    #[test]
    fn resolver_writes_virtue_innate_stats_block_after_virtues_with_defaults() {
        let db = fixture_db();
        let base_stats: HashMap<Stat, i64> = [(Stat::Will, 77)].into_iter().collect();
        let input = "\
[[item]]\n\
...
name = \"Test Helm\"\n";
        ...
        let doc: DocumentMut = out.parse().expect("output parses");
        let virtue_innate = doc
            .get(VIRTUE_INNATE_TABLE_KEY)
            .and_then(|item| item.as_table())
            .expect("VirtueInnateStats table exists");
        let keys: Vec<&str> = virtue_innate.iter().map(|(key, _)| key).collect();
        assert_eq!(
            keys,
            vec!["PhysicalMitigation", "TacticalMitigation"],
            "keys must be in canonical order"
        );
        for (_, key) in VIRTUE_INNATE_FIELDS {
            assert_eq!(
                virtue_innate.get(key).and_then(|item| item.as_integer()),
                Some(VIRTUE_INNATE_DEFAULT),
                "{key} must seed to the default"
            );
            assert_assignment_aligns_to_column_20(&out, key);
        }

        // The block sits immediately after [Virtues] and before the first item,
        // and its header note is emitted exactly once.
        let virtues_pos = out.find("[Virtues]").expect("Virtues block exists");
        let virtue_innate_pos = out
            .find("[VirtueInnateStats]")
            .expect("VirtueInnateStats block exists");
        let item_pos = out.find("[[item]]").expect("item array exists");
        assert!(
            virtues_pos < virtue_innate_pos && virtue_innate_pos < item_pos,
            "VirtueInnateStats must sit between Virtues and the first item:\n{}",
            out
        );
        assert_header_note_immediately_after(&out, "[VirtueInnateStats]", VIRTUE_INNATE_NOTE);
    }

    #[test]
    fn merge_preserves_existing_virtue_innate_values_and_restores_missing_fields() {
        let db = fixture_db();
        ...
        // Hand-edit: set PhysicalMitigation to a custom value and delete
        // TacticalMitigation entirely (to be restored to the default).
        let hand_edited = first.replace(
            "[VirtueInnateStats]\n# Virtue-passive mitigations. This assumes you have all of them maxed. If you don't, you can go look at what your actual mitigation passives are and correct the number here.\nPhysicalMitigation = 9275\nTacticalMitigation = 9275\n",
            "[VirtueInnateStats]\n# Virtue-passive mitigations. This assumes you have all of them maxed. If you don't, you can go look at what your actual mitigation passives are and correct the number here.\nPhysicalMitigation = 5000\n",
        );
        ...
        assert_eq!(
            ...
            Some(5000),
            "hand-edited value must be preserved"
        );
        assert_eq!(
            virtue_innate
                .get("TacticalMitigation")
                .and_then(|item| item.as_integer()),
            Some(VIRTUE_INNATE_DEFAULT),
            "missing field must be restored to the default"
        );
        assert_header_note_immediately_after(&merged, "[VirtueInnateStats]", VIRTUE_INNATE_NOTE);
    }
...
name = \"Test Helm\"\n";
    ...
    /// `[Virtues]` 2, `[VirtueInnateStats]` 3, items from 4 (Bug 10 territory).
    #[test]
...
name = \"Test Helm\"\n";
        ...
        let virtue_innate = position_of(VIRTUE_INNATE_TABLE_KEY);
        let first_item = doc
            .get("item")
        ...
        assert_eq!(
            virtue_innate,
            virtues + 1,
            "VirtueInnateStats must follow Virtues"
        );
        assert_eq!(
            first_item,
            virtue_innate + 1,
            "items must start right after VirtueInnateStats:\n{}",
            out
        );
        assert_canonical_header_order(&out);
    }
