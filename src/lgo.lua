...
--   /lgo probe
--     -> dump per-item API data for items in shared-storage chest 'lgo_probe' (diagnostic)
--
...
--
-- /lgo probe
--   Iterates Shared Storage and, for each item that lives in a chest named
--   'lgo_probe', dumps:
--     - everything callable on the Item object,
--     - everything callable on its ItemInfo,
--     - a deep description of itemInfo:GetCategory() and itemInfo:GetDescription().
--   Used to discover what the Turbine API actually exposes per item, in
--   particular whether crafted items can be distinguished from non-crafted.
--   Output: lgo_probe_<character>_<timestamp>.plugindata

local ITEM_METHODS = {
  "GetName", "GetQuantity", "GetMaxQuantity", "GetChest",
  "GetIcon", "GetIconImageID", "GetBackgroundIconImageID",
  "GetItemInfo", "IsItem",
};

local ITEM_INFO_METHODS = {
  "GetName", "GetCategory", "GetQuality", "GetDescription",
  "GetLevel", "GetRequiredLevel", "GetItemLevel", "GetMinLevel",
  "GetMaxStackSize", "GetItemClass",
  "GetSlot", "GetSlots",
  "IsUnique", "IsBound", "IsBoundToAccount",
  "GetBindOnAcquire", "GetBindOnEquip", "GetBinding",
  "GetIcon", "GetIconImageID",
};

-- Defensive value describer. Returns only serializable scalars/tables so
-- Turbine.PluginData.Save will not choke on engine userdata.
local function DescribeValue(v)
  ...
  local okStr, strRep = pcall(function() return tostring(v); end);
  out.tostring = okStr and strRep or "<tostring failed>";
  ...
  if t == "userdata" then
    local methodNames = {};
    local okMeta, mt = pcall(function() return getmetatable(v); end);
    if okMeta and type(mt) == "table" and type(mt.__index) == "table" then
      for k, _ in pairs(mt.__index) do
        table.insert(methodNames, tostring(k));
      end
    end
    out.metatableMethodNames = methodNames;
    -- Defensive zero-arg probes for the most likely accessor names.
    local commonProbes = { "GetName", "ToString", "GetText", "GetValue", "GetID", "GetKey" };
    local commonResults = {};
    for _, mname in ipairs(commonProbes) do
      local fn = nil;
      pcall(function() fn = v[mname]; end);
      if type(fn) == "function" then
        local okCall, result = pcall(function() return v[mname](v); end);
        commonResults[mname] = {
          ok = okCall,
    ...
    if next(commonResults) ~= nil then
      out.commonMethodResults = commonResults;
    end
    return out;
  end
  return out;
...
local function ProbeObject(obj, methodNames)
  local probe = {};
  if obj == nil then return probe end
  for _, mname in ipairs(methodNames) do
    local entry = {};
    local m = nil;
    local okGet = pcall(function() m = obj[mname]; end);
    if not okGet or type(m) ~= "function" then
      entry.existed = false;
    else
      entry.existed = true;
      local okCall, result = pcall(function() return obj[mname](obj); end);
      entry.ok = okCall;
      if okCall then
        entry.value = DescribeValue(result);
      else
...
local function ExportProbe()
  local targetChest = "lgo_probe";
  ...
  if type(ss.GetCount) ~= "function" or type(ss.GetItem) ~= "function" then
    Print("probe: shared storage API missing GetCount/GetItem; aborting");
    return;
  end

  local okCount, count = pcall(function() return ss:GetCount(); end);
  if not okCount or type(count) ~= "number" then
    Print("probe: sharedStorage:GetCount() did not return a number; aborting");
    return;
  end
  ...
  local out = {
    ...
    selectedSharedStorageChestName = targetChest,
    items = {},
  };

  local probed = 0;
  for i = 1, count do
    local okItem, item = pcall(function() return ss:GetItem(i); end);
    if okItem and item ~= nil then
      local chestIdx = nil;
      if type(item.GetChest) == "function" then
        local okC, c = pcall(function() return item:GetChest(); end);
        if okC then chestIdx = c end
      end
      local chestName = nil;
      if chestIdx ~= nil then
        chestName = GetSharedStorageChestName(ss, chestIdx);
      end

      if chestName ~= nil and Lower(chestName) == Lower(targetChest) then
        local rec = { slot = i, chest = chestIdx, chestName = chestName };

        local okName, nameVal = pcall(function() return item:GetName(); end);
        if okName and type(nameVal) == "string" and nameVal ~= "" then
          rec.name = nameVal;
        end

        rec.itemProbe = ProbeObject(item, ITEM_METHODS);

        local info = nil;
        pcall(function() info = item:GetItemInfo(); end);
        if info ~= nil then
          rec.itemInfoProbe = ProbeObject(info, ITEM_INFO_METHODS);

          pcall(function()
            local cat = info:GetCategory();
            rec.categoryReturn = DescribeValue(cat);
          end);
          pcall(function()
            local desc = info:GetDescription();
            rec.descriptionReturn = DescribeValue(desc);
          end);
          pcall(function()
            local impl = info.__implementation;
            if impl ~= nil then
              rec.implementationProbe = DescribeValue(impl);
            end
          end);
        else
          rec.itemInfoProbe = { _error = "item:GetItemInfo() returned nil or errored" };
        end
  ...
  if probed == 0 then
    Print("probe: no items found in shared-storage chest named '" .. targetChest .. "'");
    Print("probe: rename a chest to '" .. targetChest .. "' and put probe items in it, then retry");
    return;
  end

  local okSave, errSave = pcall(function() SaveAccount("lgo_probe", out); end);
  if not okSave then
    Print("probe: SaveAccount failed: " .. tostring(errSave));
    return;
  end
  Print("probe: dumped " .. tostring(probed) .. " item(s) from chest '" .. targetChest .. "'");
end
...
function Thalya.lgo.Command:Execute(command, arguments)
  ...
  if arguments == "" then
    Print("  /lgo probe                     -- diagnostic: dump full per-item API data from shared-storage chest 'lgo_probe'");
    Print("Commands:");
    Print("  /lgo ss");
