...
**Symptom:** the `Actor:GetBaseMaxMorale()` method exists in the Turbine API
but returns **the current max morale on the client** (with all equipped gear,
buffs, and virtues included), not an innate baseline. There is no API method
to read the raw innate character Morale — a permanent blind spot in the plugin
capabilities.

**Root cause:** the plugin API does not expose a `GetBaseMaxMorale()` analogue
to the Base stat methods (`GetBaseMight()`, etc.). The existing `GetMaxMorale()`
returns the live calculated total, which changes with gear and buffs.
Verified empirically by probing every callable on the `Actor` type — no
baseline method exists.

**Investigation:** CalcStat's `ClassBaseMorale(L)` formula is class-independent
and equals `RoundDbl(RoundDbl(StdProgHealth(L, 4.0), 0) * 10)`, yielding 68,000
at L160. A test case (Thalya, Lore-master, L160) confirmed:
- Measured max Morale from plugin: 187,342
- Equipped gear Morale total: 0
- Slotted virtues Morale contribution: ~47,417
- **Residual = 187,342 − 0 − 47,417 = 139,925**
- Class base from CalcStat: 68,000
- Passives/racials/tomes portion: 139,925 − 68,000 = 71,925 ✓

The residual method is sound and recovers an innate baseline close enough
for optimizer calibration.

**Fix:** the plugin now exports `maxMorale` and `maxPower` (measured from
`player:GetMaxMorale()` / `player:GetMaxPower()`, which include active buffs
at export time). `resolve-slots` populates `[MeasuredStats]` with these values
plus the equipped items list. At optimize time, the optimizer calculates
innate stats as:

```
innate_morale = measured_morale − equipment_morale − selected_virtues_morale
```

The residual is then compared against `ClassBaseMorale(level)` as a sanity
check. Users export with no buffs active for a clean baseline (or accept that
the buffs are treated as permanent by the optimizer, since they were active
at export time).

**Lesson for future agents:** when a Turbine API method name suggests a
capability ("BaseMaxMorale") but returns current state, read its docstring
or probe it empirically — names are not always truthful. And a measured
residual can be surprisingly effective for calibration: measured total −
known components = unknown remainder.
