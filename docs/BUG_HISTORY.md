...
**Follow-up fix:** prefixsearch can also return wrapper/noise pages that are not true item variants, such as `Item:Grove-tender's Robe Recipe` alongside the real `Item:Grove-tender's Robe (Item Level 564)` and `(Item Level 562)` pages. The bookmarklet now inspects prefixsearch hits before treating them as candidates: the hit must look like the exported item name, fetch successfully, expose an `Item Tooltip`, and contain usable item evidence such as a mapped slot or parsed stats. Noise pages are ignored; after filtering, all-numeric item-level candidates still auto-pick the highest level, so the Grove-tender's Robe case picks item level 564 instead of forcing `needs-pick`.
