...
- If prefixsearch returns validated candidates and *all* are tagged with `_(Item_Level_NNN)`, the bookmarklet parses out the integer levels, sorts descending, and auto-picks the highest — the equip-target for an end-game character. The chosen variant is recorded in the item record as `pickedTitle` + `pickedItemLevel` and surfaced in the result panel's "Auto-resolved via disambiguation" list and as an `# AUTO-PICKED highest-item-level variant: ...` TOML comment so the user can audit the choice.
- If exactly one validated candidate remains and it is nonnumeric, the bookmarklet auto-picks it as the only valid wiki variant and emits `# AUTO-PICKED only valid wiki variant: ...` rather than claiming a highest-item-level choice.
- If multiple validated candidates remain and any real candidate is non-numeric (`_(DPS)`, `_(Heal)`, `_(Burglar)`, etc.) — even if other candidates are numeric — the bookmarklet declines to auto-pick. The item is reported as `needs-pick`, included in the "Multiple variants exist; auto-pick declined" sub-list of the result panel, and emitted into the TOML with all-zero stats and a `# UNRESOLVED: ...` comment.
...
**Follow-up fix:** prefixsearch can also return wrapper/noise pages that are not true item variants, such as `Item:Grove-tender's Robe Recipe` alongside the real `Item:Grove-tender's Robe (Item Level 564)` and `(Item Level 562)` pages. The bookmarklet now fetches prefixsearch hits before treating them as candidates: the page must match the exported item name either by normalized title base or by tooltip display name, expose an `Item Tooltip` when inspected, and contain usable item evidence such as a mapped slot or parsed stats. Noise pages are ignored; after filtering, all-numeric item-level candidates still auto-pick the highest level, so the Grove-tender's Robe case picks item level 564 instead of forcing `needs-pick`.
