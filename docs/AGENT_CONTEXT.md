...
1. A **Lua in-game plugin** (`src/lgo.lua`) that exports the player's equipped gear plus the contents of a Shared Storage chest named `lgo`, writing one file to `Documents\The Lord of the Rings Online\PluginData\<account>\AllServers\`:
   - `lgo_<character-name>_gearNames_<timestamp>.plugindata` — a flat list of equipped + chest item names, plus the character's class, base stats, level, measured max Morale/Power, active-effect count, and equipped-item list (input for the bookmarklet; used by resolve-slots for measured-residual innate baseline calibration).
...
- User clicks **Save TOML...** to save the bookmarklet's output as: lgo_`<character>`_gearStats.toml, into the character's `<install>\<CharacterName>_Gear\` folder (the install directory is the folder containing `lgo.exe`), or into the install root on the first run — a root-saved file is moved into the character folder by `resolve-slots`.
  - **For a clean innate baseline:** export the plugindata with no active food, hope, or fellowship buffs. The measured Max Morale/Power will then reflect equipped gear only, without buff contributions.
- User invokes 'lgo resolve-slots' to merge that list into a file named: lgo_`<character>`_gearReady.toml, in the same `<CharacterName>_Gear\` folder (created if missing). The top-level `level` field and `[MeasuredStats]` block (containing measured Max Morale/Power, active effects, and equipped-item list) are generated by resolve-slots and regenerated wholesale on every run — do not hand-edit them.
...
The file begins with a top header containing `character`, `class`, `level` (integer, measured character level; generated and regenerated on every `resolve-slots` run), `[MeasuredStats]`, `[InnateStats]`, and `[Virtues]` as the last pre-items blocks.

`[MeasuredStats]` is generated by `resolve-slots` from the plugindata export and is regenerated wholesale on every run:
- `MaxMorale` — measured Max Morale at export (includes equipped gear and active buffs)
- `MaxPower` — measured Max Power at export (includes equipped gear and active buffs)
- `ActiveEffects` — count of active effects at export (0 if none)
- `Equipped` — array of strings, names of equipped items in slot order (used by the innate-baseline calibration)

Do not hand-edit `level` or `[MeasuredStats]`; they are generated.

`[InnateStats]` holds only the five raw Base stats (`Might`, `Agility`, `Vitality`, `Will`, `Fate`), passed through verbatim from the plugindata by `resolve-slots`. `[Virtues]` holds five user-maintained string slots (`Virtue1` ... `Virtue5`) whose non-empty values are matched case-insensitively against the top-level keys in `data/lgo_virtues.json`. After that, the format per item is:
...
- **`Actor:GetBaseMaxMorale()` returns the character's *current* Max Morale on this client, which includes equipped gear, active buffs, and virtues — it is not the innate baseline and is useless for calibration.** The only reliable way to measure the innate Morale/Power baseline is via the residual method: measured Max (from export) minus known equipped-gear contributions minus selected Virtue contributions minus derived Base-stat contributions.
- **Virtue *passives* (all earned virtues, slotted or not) add Morale / PhysMit / TacMit and are not exposed by the Turbine API.** They are not modelled; they are absorbed into the measured innate baseline along with racials and stat tomes. This is an accepted permanent limitation: virtue passives are a blind spot.
- **CalcStat class-independence:** the formula `ClassBaseMorale(L) = RoundDbl(RoundDbl(StdProgHealth(L, 4.0), 0) * 10)` is class-independent, by design. At L160, it yields exactly 68,000 Morale; this can be cross-validated via the Vitality formula `ClassBaseVitality(L) = L × 60 + 600`, which at L160 yields 10,200, exactly matching the exported `[InnateStats]` value. This formula is historical research input only; LGO does not implement or import CalcStat. Source data lives on the `CalcStat` branch under `docs/CalcStat/`; it is deliberately not merged to `main`.
- **Character panel rounds Fate×1.5 half-down (3219×1.5 → 4828.5 → 4828),** whereas LGO's per-item derivation uses `ceil()`. This produces an expected ±1 discrepancy in Fate-derived stats (Fate Resilience, Fate Mitigation). Not a bug; it is accepted and documented.
