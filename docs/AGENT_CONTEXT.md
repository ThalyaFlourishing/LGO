...
- `src/bin/db_build.rs` — offline database builder. Reads data/items.xml + data/progressions.xml, writes data/lgo_items.json. Run via cargo run --bin db_build --release after deleting the JSON. Skipped if the JSON already exists.
