...
1. A **Lua in-game plugin** (`src/lgo.lua`) that exports the player's equipped gear plus the contents of a Shared Storage chest named `lgo`, writing one file to `Documents\The Lord of the Rings Online\PluginData\<account>\AllServers\`:
   - `lgo_<character-name>_gearNames_<timestamp>.plugindata` — a flat list of equipped + chest item names, plus the character's class, level, measured max Morale/Power, active-effect count, and base stats (input for the bookmarklet).
...
The short form:
- User exports a file from the in-game plug-in: `lgo_<character-name>_gearNames_<timestamp>.plugindata` (optionally with no food/hope/fellowship buffs active for a clean Morale/Power baseline).
...
- User invokes 'lgo resolve-slots' to merge that list into a file named: lgo_`<character>`_gearReady.toml, in the same `<CharacterName>_Gear\` folder (created if missing). This step generates/regenerates `level` and `[MeasuredStats]` from the plugindata.
- User hand-edits persistent corrections, including top-level `[Virtues]` and
  per-item essence totals, in `gearReady.toml` only. **Do not hand-edit `level` or `[MeasuredStats]`** — these are regenerated on every `resolve-slots`.
...
`level` is an integer (`160`), derived from the plugindata and regenerated on every
`resolve-slots`. `[InnateStats]` holds only the five raw Base stats (`Might`, 
`Agility`, `Vitality`, `Will`, `Fate`), passed through verbatim from the plugindata 
by `resolve-slots`. `[MeasuredStats]` is a **generated, regenerated wholesale on 
every `resolve-slots`, never hand-edited** table containing `MaxMorale` (integer), 
`MaxPower` (integer), `ActiveEffects` (integer), and `Equipped` (array of strings). 
...
[MeasuredStats]
# Generated by resolve-slots; do not hand-edit.
MaxMorale          = 187342
MaxPower           = 21005
ActiveEffects      = 0
Equipped           = ["item1", "item2", ...]
...
- **`Actor:GetBaseMaxMorale()` returns the *current* max morale on the current client, not an innate baseline.** It includes equipped gear, buffs, and virtues — entirely useless for measuring innate stats. The measured-residual method (max Morale from plugindata minus equipped gear stats via lookup) is the workaround.
- **Virtue *passives* (all earned virtues, slotted or not) add Morale, Physical Mitigation, and Tactical Mitigation,** but the Turbine plugin API exposes only selected virtues (from `[Virtues]` in the TOML), not earned-but-unslotted ones. This is a permanent blind spot: the residual method can detect only the slotted portion of virtue passives.
- **CalcStat's `ClassBaseMorale(L)` is class-independent:** it equals `RoundDbl(RoundDbl(StdProgHealth(L, 4.0), 0) * 10)`. At L160, this formula yields 68,000. Validated empirically by cross-checking with `ClassBaseVitality(160) = 10,200`, which matches the test export exactly — strong evidence the residual method is sound.
- **The character panel rounds Fate×1.5 half-down** (e.g. 3219×1.5 → 4828), whereas LGO's per-item stat rounding rule uses `ceil`. This produces a known ±1 discrepancy in Fate-derived stats (e.g. Morale). Documented for transparency; no fix warranted.
