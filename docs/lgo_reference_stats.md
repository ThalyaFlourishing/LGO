...
Note: `Block`, `Parry`, and `Evade` have no known XML stat name in
`items.xml`. They are defined in the enum for future use and manual
entry via the stats file only.